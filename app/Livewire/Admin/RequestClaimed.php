...
class RequestClaimed extends Component implements HasForms, HasTable
{
    ...
    {
        return $table
            ->query(auth()->user()->role_id === 1 ? RequestModel::query()->where('status', 'Claimed')->orderBy('created_at', 'desc') :
            RequestModel::query()->where('status', 'Claimed')->whereHas('user_information', function($query) {
            ...
            ->emptyStateDescription('All claimed requests will be displayed here');;
    }

    public function render()
    {
        return view('livewire.admin.request-claimed');
    }
}
